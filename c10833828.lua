...
function s.spfilter3(c,e,tp,m,g,f,chkf)
	return c:IsType(TYPE_FUSION) and c:IsRace(RACE_FIEND) and (not f or f(c))
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false) and g:IsExists(s.spfilter4,1,nil,m,c,chkf)
end
function s.spfilter4(c,m,fusc,chkf)
...
function s.sptg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local res=Duel.IsExistingMatchingCard(s.spfilter3,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,g,nil,chkf)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(s.spfilter3,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,g,mf,chkf)
			end
		end
		return res
	end
...
function s.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg1=Duel.GetMatchingGroup(s.spfilter3,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,g,nil,chkf)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(s.spfilter3,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,g,mf,chkf)
	end
	if #sg1>0 or (sg2~=nil and #sg2>0) then
		...
		if sg1:IsContains(tc) and (sg2==nil or not sg2:IsContains(tc) or not Duel.SelectYesNo(tp,ce:GetDescription())) then
			...
			local ec=g:FilterSelect(tp,s.spfilter4,1,1,nil,mg1,tc,chkf):GetFirst()
			...
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			local ec=g:FilterSelect(tp,s.spfilter4,1,1,nil,mg2,tc,chkf):GetFirst()
