...
function s.op(e,tp,eg,ep,ev,re,r,rp)
	...
	s.VictoryEffects(dm[0])
end

function s.VictoryEffects(c)
...
function s.loseop2(e,tp,eg,ep,ev,re,r,rp)
...
function s.pass1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return eg:IsExists(s.abcon,1,nil,tp) end
	local g=eg:Filter(s.abcon,nil,tp)
	if #g>0 then
		local tc=g:GetFirst()
		while tc do
			if not tc:IsReason(REASON_SPSUMMON) then
				s.pass2(e,tp,Group.FromCards(tc),ep,ev,re,r,rp)
			else
...
function s.pass2(e,tp,eg,ep,ev,re,r,rp)
	if chk==0 then return eg:IsExists(s.abcon2,1,nil,tp) end
	local g=eg:Filter(s.abcon2,nil,tp)
	if #g>0 then
		local tc=g:GetFirst()
		while tc do
			tc:ResetFlagEffect(id)
...
function s.checkop(e,tp,eg,ep,ev,re,r,rp)
	for i=0,1 do
		if getmatchgc(Card.IsDeckMaster,i,0xff,0,nil)==0 and Duel.GetFlagEffect(i,id+1)>0 then
			local g=eg:Filter(s.filterdm,nil,i)
			if #g>0 then
			local tc=g:GetFirst()
				while tc do
...
function s.RemoveDeckMasters(g1)
