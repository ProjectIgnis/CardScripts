...
if not ActionDuel then
		...
		for c in aux.Next(g) do
			...
			if not e:GetLabelObject() then
				...
				for _,tc in ipairs({c,Duel.CreateToken(1-tp,c:GetOriginalCode())}) do
					...
					if Duel.IsExistingMatchingCard(Card.IsCode,tp,0xff,0xff,1,nil,511004400) then
	...
	function ActionDuel.operation(e,tp,eg,ep,ev,re,r,rp)
		...
		if Duel.IsExistingMatchingCard(ActionDuel.acfilter,tokenp,LOCATION_HAND,0,2,nil) and Duel.Remove(token,POS_FACEUP,REASON_EFFECT) then
