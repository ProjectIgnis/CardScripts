...
function s.initial_effect(c)
	if not s.global_check then
		...
		atkeff:SetOperation(s.atkraiseeff)
		...
		atkadj:SetOperation(s.atkraiseadj)
		...
		arm1:SetCondition(s.armatkcon)
		arm1:SetTarget(s.armatktg)
		...
		arm2:SetOperation(s.armcheckop)
		...
		arm3:SetCondition(s.armatcon)
		arm3:SetOperation(s.armatop)
...
function s.atkraiseeff(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(s.atkcfilter,tp,0x7f,0x7f,nil)
	...
	local dg=Duel.GetMatchingGroup(s.defcfilter,tp,0x7f,0x7f,nil)
	...
	local lvg=Duel.GetMatchingGroup(s.lvcfilter,tp,0x7f,0x7f,nil)
...
function s.atkraiseadj(e,tp,eg,ep,ev,re,r,rp)
...
function s.cardianchk(c,tp,eg,ep,ev,re,r,rp)
...
function s.armatcon(e,tp,eg,ep,ev,re,r,rp)
...
function s.armatop(e,tp,eg,ep,ev,re,r,rp)
	local atg=Duel.GetAttacker():GetAttackableTarget()
	local d=Duel.GetAttackTarget()
	if atg:IsExists(s.armfilter,1,d) and Duel.SelectYesNo(tp,aux.Stringid(21558682,0)) then
		local g=atg:FilterSelect(tp,s.armfilter,1,1,d)
		Duel.HintSelection(g)
		Duel.ChangeAttackTarget(g:GetFirst())
	end
end

function s.batregop(e,tp,eg,ep,ev,re,r,rp)
	IndesTable={}
	local a=Duel.GetAttacker()
	local d=Duel.GetAttackTarget()
	if a then
...
function s.batend(e,tp,eg,ep,ev,re,r,rp)
...
function s.relayop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLP(0)<=0 and not Duel.IsPlayerAffectedByEffect(0,EFFECT_CANNOT_LOSE_LP) and Duel.GetFlagEffect(0,511002521)==0 
		...
		ge2:SetLabel(0)
		ge2:SetOperation(s.relayresetop)
...
function s.relayresetop(e,tp,eg,ep,ev,re,r,rp)
