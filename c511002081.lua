...
function s.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>1 and not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT) 
		and Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0,TYPES_TOKEN,0,0,1,RACE_WINDBEAST,ATTRIBUTE_DARK) end
...
function s.tkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0,TYPES_TOKEN,0,0,1,RACE_WINDBEAST,ATTRIBUTE_DARK) then return end
	for i=1,2 do
...
function s.synchk(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		...
		if tc.synchro_type==1 then
			...
			e1:SetCondition(aux.SynCondition(f1,min1,max1,f2,min2,max2,sub1,sub2,req1,req2,s.reqm(reqm)))
			e1:SetTarget(aux.SynTarget(f1,min1,max1,f2,min2,max2,sub1,sub2,req1,req2,s.reqm(reqm)))
		...
		elseif tc.synchro_type==2 then
			...
			e1:SetProperty(EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_IGNORE_IMMUNE)
			e1:SetRange(LOCATION_GRAVE)
			e1:SetCondition(aux.SynCondition(table.unpack(t),s.reqm()))
			e1:SetTarget(aux.SynTarget(table.unpack(t),s.reqm()))
			e1:SetOperation(aux.SynOperation)
			e1:SetValue(SUMMON_TYPE_SYNCHRO)
			tc:RegisterEffect(e1)
