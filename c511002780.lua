...
function s.filter1(c,e,tp)
	return c:IsFaceup() and c:IsRace(RACE_MACHINE) and c:GetLevel()==3
		and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_HAND,0,1,nil,e,tp,c:GetCode())
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsFaceup() and tc:IsRelateToEffect(e) then
		...
		local g=Duel.SelectMatchingCard(tp,s.filter2,tp,LOCATION_HAND,0,1,1,nil,e,tp,code)
		if #g>0 then
