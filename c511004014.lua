...
function s.op(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetFlagEffect(tp,id)==0 and Duel.GetFlagEffect(1-tp,id)==0 then
		...
		e6:SetTarget(function(e,c) return c:IsStatus(STATUS_SPSUMMON_TURN) and (c:IsSummonLocation(LOCATION_EXTRA) or (c:IsAttribute(ATTRIBUTE_DEVINE) and c:IsSummonLocation(LOCATION_GRAVE))) and c:GetFlagEffect(511004016)==0 end)
