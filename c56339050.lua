...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if Duel.CheckEvent(EVENT_ATTACK_ANNOUNCE) and Duel.GetTurnPlayer()~=tp then
		...
		if Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_MZONE,0,1,nil,tc:GetAttack())
...
function s.target2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return e:GetHandler():GetFlagEffect(id)==0	and tc:IsOnField() and tc:IsCanBeEffectTarget(e) end
	Duel.SetTargetCard(tc)
	local dam=tc:GetAttack()/2
	Duel.SetTargetParam(dam)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,tc,1,0,0)
