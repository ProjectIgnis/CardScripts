...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return 
		Duel.IsExistingMatchingCard(s.filter1,tp,0,LOCATION_MZONE,1,nil) or
		Duel.IsExistingMatchingCard(s.filter2,tp,0,LOCATION_ONFIELD,1,nil) or
		Duel.IsExistingMatchingCard(s.filter3,tp,0,LOCATION_SZONE,1,nil)
	end
	local t={}
	local p=1
	if Duel.IsExistingMatchingCard(s.filter1,tp,0,LOCATION_MZONE,1,nil) then t[p]=aux.Stringid(id,0) p=p+1 end
	if Duel.IsExistingMatchingCard(s.filter2,tp,0,LOCATION_ONFIELD,1,nil) then t[p]=aux.Stringid(id,1) p=p+1 end
	if Duel.IsExistingMatchingCard(s.filter3,tp,0,LOCATION_SZONE,1,nil) then t[p]=aux.Stringid(id,2) p=p+1 end
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(id,3))
	local sel=Duel.SelectOption(tp,table.unpack(t))+1
	local opt=t[sel]-aux.Stringid(id,0)
	local sg=nil
	if opt==0 then sg=Duel.GetMatchingGroup(s.filter1,tp,0,LOCATION_MZONE,nil)
