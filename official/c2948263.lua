...
function s.spcon(e,c)
    ...
    return Duel.CheckReleaseGroup(c:GetControler(),Card.IsSetCard,1,true,1,true,nil,c,c:GetControler(),e:GetHandler(),0x59)
end
function s.sptg(e,tp,eg,ep,ev,re,r,rp,c)
    local g=Duel.SelectReleaseGroup(tp,Card.IsSetCard,1,1,true,true,true,c,nil,nil,e:GetHandler(),0x59)
    if g then
        g:KeepAlive()
        e:SetLabelObject(g)
    return true
