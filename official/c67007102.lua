...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsCode,CARD_GOLDEN_LORD),tp,LOCATION_ONFIELD,0,1,nil)
		and Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.HasNonZeroAttack),tp,LOCATION_MZONE,LOCATION_MZONE,1,nil)
		and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
		Duel.BreakEffect()
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
		local g=Duel.SelectMatchingCard(tp,aux.FilterFaceupFunction(Card.HasNonZeroAttack),tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
		local tc=g:GetFirst()
		local e1=Effect.CreateEffect(c)
