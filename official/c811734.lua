...
function s.atkfilter(c)
	return c:IsSetCard(SET_DDD) and c:GetAttack()>0 and c:IsAbleToRemove() and aux.SpElimFilter(c,true)
end
function s.thfilter(c)
	return c:IsLevelBelow(4) and c:IsSetCard(SET_DD) and c:IsType(TYPE_PENDULUM) and c:IsAbleToHand()
end
