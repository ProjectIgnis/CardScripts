...
s.listed_series={SET_HEROIC_CHALLENGER}
s.listed_names={id}
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsTurnPlayer(tp)
end
function s.filter(c,e,tp)
	return not c:IsCode(id) and c:IsSetCard(SET_HEROIC_CHALLENGER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.lvfilter(c)
	return c:IsFaceup() and c:IsSetCard(SET_HEROIC_CHALLENGER) and c:GetLevel()>1
end
