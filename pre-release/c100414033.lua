...
s.listed_series={0x242}
s.listed_names={CARD_GOLDEN_LORD}
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0x242,0x11,1800,1500,5,RACE_ZOMBIE,ATTRIBUTE_LIGHT) end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,id,0x242,0x11,1800,1500,5,RACE_ZOMBIE,ATTRIBUTE_LIGHT) then return end
...
function s.setfilter(c)
	return c:IsSetCard(0x242) and c:IsSSetable()
end
