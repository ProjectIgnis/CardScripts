...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	local ec=Duel.SelectMatchingCard(tp,Card.IsSetCard,tp,LOCATION_EXTRA,0,1,1,nil,0x24b):GetFirst()
	if ec then
...
function s.eqfilter(c)
	return c:IsOriginalType(TYPE_MONSTER) and c:IsSetCard(0x24b)
end
function s.spcond(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(s.eqfilter,1,nil)
end
function s.spfilter(c,e,tp)
	return c:IsRace(RACE_SPELLCASTER) and c:IsLevel(4) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)
end
