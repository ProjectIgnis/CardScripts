...
function s.initial_effect(c)
	--Activate 1 of these effects (but you can only use each effect of "Hecatoncheire Tartarus" once per turn)
	...
	c:RegisterEffect(e1)
	--If you use a face-up monster(s) you control owned by your opponent, you can treat it as a "Hecatoncheire" monster
	aux.GlobalCheck(s,function()
		...
		e0a:SetValue(SET_HECATONCHEIRE)
...
function s.effcost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(-100)
	local fusion_params={handler=e:GetHandler(),fusfilter=aux.FilterBoolFunction(Card.IsSetCard,SET_HECATONCHEIRE),matfilter=Fusion.OnFieldMat(Card.IsAbleToRemove),extrafil=s.fextra,extraop=Fusion.BanishMaterial,extratg=s.extratg}
	local b1=not Duel.HasFlagEffect(tp,id)
		and Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsSetCard,SET_HECATONCHEIRE),tp,LOCATION_MZONE,0,1,nil)
...
function s.effop(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif op==2 then
		--Fusion Summon 1 "Hecatoncheire" Fusion Monster from your Extra Deck, by banishing its materials from your field and/or GY
		local fusion_params={handler=e:GetHandler(),fusfilter=aux.FilterBoolFunction(Card.IsSetCard,SET_HECATONCHEIRE),matfilter=Fusion.OnFieldMat(Card.IsAbleToRemove),extrafil=s.fextra,extraop=Fusion.BanishMaterial,extratg=s.extratg}
