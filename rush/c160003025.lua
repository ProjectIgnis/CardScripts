...
function s.costfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsRace(RACE_DRAGON) and c:IsLevelAbove(7) and c:IsAbleToDeckAsCost()
end
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.costfilter,tp,LOCATION_HAND,0,1,nil) end
end
function s.atkfilter(c)
	return c:IsType(TYPE_MONSTER) and Ic:IsFaceup() and c:GetAttack()>0
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoGrave(tg,nil,REASON_COST)==1 then
		...
		if #g>0 then
			...
			local g2=Duel.SelectMatchingCard(tp,s.atkfilter,tp,0,LOCATION_MZONE,1,2,nil)
			if #g2>0 then
