...
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if a and a:IsAttackPos() and a:IsRelateToBattle() then
		...
		if Duel.IsExistingMatchingCard(s.atkfilter2,tp,0,LOCATION_MZONE,1,nil,tp) and Duel.GetFieldGroupCount(tp,0,LOCATION_SZONE)>0 and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
			Duel.BreakEffect()
