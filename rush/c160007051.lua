...
function s.drtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.pyrofilter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
end
function s.drop(e,tp,eg,ep,ev,re,r,rp)
	-- Requirement
	local pg=Duel.GetMatchingGroup(s.pyrofilter,tp,LOCATION_GRAVE,0,nil)
	...
	if dr>0 and Duel.Draw(tp,dr,REASON_EFFECT)>0
		and Duel.IsExistingMatchingCard(s.pyrofilter,tp,LOCATION_MZONE,0,1,nil)
