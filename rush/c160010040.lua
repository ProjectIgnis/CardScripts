...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.CanAttack),tp,LOCATION_MZONE,0,1,nil) end
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg=Duel.SelectMatchingCard(tp,aux.FilterFaceupFunction(Card.CanAttack),tp,LOCATION_MZONE,0,1,1,nil)
	if #sg>0 then
