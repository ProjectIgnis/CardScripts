...
function s.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoGrave(tc,REASON_COST)==1 and Duel.Draw(tp,1,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_GRAVE) and Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsLevel,7),tp,0,LOCATION_MZONE,1,nil) and Duel.IsPlayerCanDraw(tp,1) and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then 
		Duel.BreakEffect()
