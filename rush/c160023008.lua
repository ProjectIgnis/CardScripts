...
function s.revealfilter(c)
	return c:IsRace(RACE_HIGHDRAGON) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsType(TYPE_FUSION) and c.material and not c:IsPublic()
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetValue(code)
	c:RegisterEffect(e1)
	--Prevent non-Dragons from attacking
...
function s.ftarget(e,c)
	return not c:IsRace(RACE_HIGHDRAGON|RACE_DRAGON)
end