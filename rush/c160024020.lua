...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		...
		if Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,160004013),tp,LOCATION_MZONE,0,1,nil) and #sg>0 and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_POSCHANGE)
			local sc=Group.Select(sg,tp,1,2,nil)
