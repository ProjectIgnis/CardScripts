--火霊術師の使い魔
--Familiar of the Fire Spiritualist
--scripted by YoshiDuels
local s,id=GetID()
function s.initial_effect(c)
	--Change selected monster's attribute to FIRE
...
function s.attfilter(c)
	return c:IsFaceup() and c:CanChangeIntoAttributeRush(ATTRIBUTE_FIRE) and c:IsNotMaximumModeSide()
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.attfilter,tp,0,LOCATION_MZONE,1,nil) end
end
function s.cfilter(c)
	return c:IsCode(759393) and c:IsFacedown()
end
