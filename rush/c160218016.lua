--風霊術師の使い魔
--Familiar of the Wind Spiritualist
--scripted by YoshiDuels
local s,id=GetID()
function s.initial_effect(c)
	--Change selected monster's attribute to WIND
...
function s.attfilter(c)
	return c:IsFaceup() and c:CanChangeIntoAttributeRush(ATTRIBUTE_WIND) and c:IsNotMaximumModeSide()
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.attfilter,tp,0,LOCATION_MZONE,1,nil) end
end
function s.cfilter(c)
	return c:IsCode(37744402) and c:IsFacedown()
end
