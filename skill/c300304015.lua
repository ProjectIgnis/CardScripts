...
function s.cyberdarkfilter(c)
	return c:IsSetCard(SET_CYBERDARK) and c:IsMonster() and c:IsAbleToHand()
end
function s.flipcon(e,tp,eg,ep,ev,re,r,rp)
	local b1=Duel.GetFlagEffect(tp,id)==0 and Duel.GetLP(tp)<=3000
	local b2=Duel.GetFlagEffect(tp,id+100)==0 and Duel.GetLP(tp)<=2000
	local b3=Duel.GetFlagEffect(tp,id+200)==0 and Duel.GetLP(tp)<=1000
	return aux.CanActivateSkill(tp) and Duel.IsExistingMatchingCard(s.cyberdarkfilter,tp,LOCATION_DECK,0,3,nil) and (b1 or b2 or b3)
end
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
	...
	if b1 and not (b2 or b3) then
		...
		local g=Duel.GetMatchingGroup(s.cyberdarkfilter,tp,LOCATION_DECK,0,nil)
	...
	elseif (b1 or b2) and not b3 then
		Duel.Hint(HINT_SKILL_FLIP,tp,id|(1<<32))
		Duel.Hint(HINT_CARD,tp,id)
