...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
	...
	e3:SetRange(0x5f)
	e3:SetCondition(function(e) return Duel.IsExistingMatchingCard(aux.AND(Card.IsTributeSummoned,Card.IsReleasable),e:GetHandlerPlayer(),LOCATION_MZONE,0,1,nil) end)
...
function s.otcon(e,c,minc)
	...
	return minc<=2 and Duel.IsExistingMatchingCard(aux.AND(Card.IsTributeSummoned,Card.IsReleasable),tp,LOCATION_MZONE,0,1,nil)
end
function s.ottg(e,c)
...
function s.sumtg(e,tp,eg,ep,ev,re,r,rp,c)
	...
	local g=Duel.SelectMatchingCard(tp,aux.AND(Card.IsTributeSummoned,Card.IsReleasable),tp,LOCATION_MZONE,0,1,1,true,nil)
	if g then
		g:KeepAlive()
		e:SetLabelObject(g)
		return true
