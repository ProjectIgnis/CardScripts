...
function s.spcon(e,c)
    ...
    return Duel.CheckReleaseGroup(c:GetControler(),s.filter,1,false,1,true,nil,c,c:GetControler(),nil)
end
function s.sptg(e,tp,eg,ep,ev,re,r,rp,c)
    local g=Duel.SelectReleaseGroup(tp,s.filter,1,1,false,true,true,c,nil,nil,nil)
    if g then
        g:KeepAlive()
        e:SetLabelObject(g)
    return true
