...
function s.filter2(c,e,tp,m,f)
	return c:IsType(TYPE_FUSION) and c.miracle_synchro_fusion and (not f or f(c))
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false) and m:IsExists(s.filter3,1,nil,c,m,tp)
end
function s.filter3(c,fusc,m,tp)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if #sg1>0 or (sg2~=nil and #sg2>0) then
		...
		if sg1:IsContains(tc) and (sg2==nil or not sg2:IsContains(tc) or not Duel.SelectYesNo(tp,ce:GetDescription())) then
			...
			local gc=mg1:FilterSelect(tp,s.filter3,1,1,nil,tc,mg1,tp):GetFirst()
			...
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			local gc=mg3:FilterSelect(tp,s.filter3,1,1,nil,tc,mg3,tp):GetFirst()
