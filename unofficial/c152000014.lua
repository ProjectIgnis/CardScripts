...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
	--OPD Check and asks if you want to activate the Skill or not
	if Duel.GetFlagEffect(tp,id)>0 or not Duel.SelectYesNo(tp,aux.Stringid(id,0)) then return end
	--OPD register
	Duel.RegisterFlagEffect(tp,id,0,0,0)
	Duel.Hint(HINT_SKILL_FLIP,tp,id|(1<<32))
	Duel.Hint(HINT_CARD,tp,id)
	--Check if Skill is negated ("Anti-Skill")
	if aux.CheckSkillNegation(e,tp) then return end
	--Recover LP equal to the damage taken from that direct attack, then Set 1 Trap Card from outside the Duel to your field.
