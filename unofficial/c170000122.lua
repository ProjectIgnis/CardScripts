...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return e:IsHasType(EFFECT_TYPE_ACTIVATE) 
		and Duel.IsExistingTarget(aux.FilterFaceupFunction(aux.NOT(Card.IsType),TYPE_PLUS),tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,aux.FilterFaceupFunction(aux.NOT(Card.IsType),TYPE_PLUS),tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
...
function s.becon(e)
	return Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsType,TYPE_MINUS),e:GetHandler():GetEquipTarget():GetControler(),0,LOCATION_MZONE,1,nil)
end
