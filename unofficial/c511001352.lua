...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Give control of any Level 4 or lower Insect monster you control to your opponent
...
function s.ctrlop(e,tp,eg,ep,ev,re,r,rp)
	...
	local oft=Duel.GetMZoneCount(1-tp)
	if #g==0 or oft<=0 then return end 
	if Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
		Duel.Hint(HINT_CARD,0,id)
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONTROL)
		local sg=g:FilterSelect(tp,s.ctrlfilter,1,oft,nil)
		if #sg>0 then
