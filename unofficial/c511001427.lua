...
function s.filter(c,tp,tid)
	...
	return no and no>=101 and no<=107 and c:IsSetCard(0x1048)
		...
		and c:GetControler()==c:GetPreviousControler()
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.GetMatchingGroup(s.filter,tp,LOCATION_GRAVE,0,nil,tp,tid):Filter(Card.IsCanBeSpecialSummoned,nil,e,tp,false,false,POS_FACEUP)
	local ct=#g
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(s.filter,tp,LOCATION_GRAVE,0,nil,tp,Duel.GetTurnCount()):Filter(Card.IsCanBeSpecialSummoned,nil,e,tp,false,false,POS_FACEUP)
	local ct=#g
