...
function s.op(oc)
	...
	--Monsters Special Summoned from the Extra Deck can be treated as tributes for a Tribute Summon up to the number of Material(s) used for their Summon (2 Materials/Double Tribute)
	...
	e2a:SetTarget(function(e,c)return c:IsSummonLocation(LOCATION_EXTRA) and c:GetMaterialCount()==2 end)
	e2a:SetValue(1)
	Duel.RegisterEffect(e2a,0)
	--Monsters Special Summoned from the Extra Deck can be treated as tributes for a Tribute Summon up to the number of Material(s) used for their Summon (3 Materials/Triple Tribute)
	local e2a=Effect.GlobalEffect()
	e2a:SetType(EFFECT_TYPE_FIELD)
	e2a:SetProperty(EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_IGNORE_IMMUNE)
	...
	e3:SetCode(EFFECT_NORMAL_SUMMON_FACEUP_DEFENSE)
	...
	--When an activated Spell/Trap is destroyed by a card effect, the effect of the Spell/Trap destroyed is also negated.
	...
	--If a monster(s) is Summoned to the attacked player's field when an attack is declared, the Summoning player can change the attack target to 1 of the Summoned monsters. If a monster(s) is Summoned just before the resolution of an effect that targets exactly 1 monster on the same side of the field, the Summoning player can change the effect target to 1 of the Summoned monsters.
	...
	e10a:SetCode(EVENT_SPSUMMON_SUCCESS)
	Duel.RegisterEffect(e10a,0)
	local e10b=e10:Clone()
	e10b:SetCode(EVENT_FLIP_SUMMON_SUCCESS)
	Duel.RegisterEffect(e10b,0)
	--If a monster declares an attack and loses its attack target, then that attack is negated.
	...
	--If both players agree to apply Manga Rules:
	if Duel.SelectYesNo(0,aux.Stringid(id,0)) and Duel.SelectYesNo(1,aux.Stringid(id,0)) then
		...
		--Each player can have up to 7 cards in their hands.
		...
		--The total number of cards that each player can have on the field is 5.
		local e13=e12:Clone()
		...
		--Each player can only activate 1 Spell from their hand, set 1 Spell and 1 Trap per turn.
		local e15=e12:Clone()
		...
		-- Monster Cards cannot be Set or flipped face-down.
		...
		--All Traps can be activated the turn they are Set.
		local e22=e19:Clone()
