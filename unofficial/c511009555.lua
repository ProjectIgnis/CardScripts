...
function s.filter1(c,e,tp)
	...
	return #pg<=1 and c:IsFaceup() and c:IsSetCard(SET_RAIDRAPTOR) and (rk>0 or c:IsStatus(STATUS_NO_LEVEL))
...
function s.filter2(c,e,tp,mc,rk,pg)
	...
	return c:IsType(TYPE_XYZ) and mc:IsType(TYPE_XYZ,c,SUMMON_TYPE_XYZ,tp)
		and c:IsRank(rk) and c:IsSetCard(SET_RAIDRAPTOR) and mc:IsCanBeXyzMaterial(c,tp)
