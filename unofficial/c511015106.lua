...
function s.oddeyesspfilter(c,e,tp)
	return c:IsCode(16178681) and c:IsFaceup() and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		...
		and Duel.IsExistingTarget(s.xyzgyspfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp,c)
end
function s.xyzgyspfilter(c,e,tp,odd)
	return c:IsXyzMonster() and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		and Duel.IsExistingMatchingCard(s.xyzedspfilter,tp,LOCATION_EXTRA,0,1,odd,e,tp,c,odd)
end
function s.xyzedspfilte(c,e,tp,xyz,odd)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local c2=Duel.SelectTarget(tp,s.xyzgyspfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp,c1):GetFirst()
...
function s.atkcon(e,tp,eg,ev,ep,re,r,rp)
	return Duel.IsTurnPlayer(tp) and Duel.IsBattlePhase() and aux.StatChangeDamageStepCondition()
end
