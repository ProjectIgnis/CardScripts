...
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetAttacker():IsControler(1-tp) and Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_EMZONE,0,1,nil)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc==Duel.GetAttacker() end
	local tg=Duel.GetAttacker()
	local zone=aux.GetMMZonesPointedTo(tp,s.cfilter,LOCATION_EMZONE,0,1-tp)
