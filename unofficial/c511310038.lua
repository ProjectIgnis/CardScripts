...
function s.cfilter(c,tp)
	return c:IsSetCard(0x10b) and c:IsAbleToGrave() and not table.includes(s.name_list[tp],c:GetCode())
end
...
function s.posop(e,tp,eg,ep,ev,re,r,rp)
	...
	if dc and Duel.SendtoGrave(dc,REASON_EFFECT)~=0 and dc:IsLocation(LOCATION_GRAVE) then
		Duel.BreakEffect()
