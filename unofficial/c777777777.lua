...
function s.initial_effect(c)
    aux.GlobalCheck(s,function()
		...
		e3:SetProperty(EFFECT_FLAG_IGNORE_IMMUNE+EFFECT_FLAG_SINGLE_RANGE+EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_CANNOT_DISABLE)
...
if not c44330098 then
	...
	function c44330098.filter(c)
		return not c:IsStatus(STATUS_LEAVE_CONFIRMED) and c:IsLocation(LOCATION_FZONE)
	end
...
if not c44790889 then
	...
	function c44790889.ntcon(e,c,minc)
		...
		return minc==0 and c:GetLevel()>4 and Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0
			and Duel.GetFieldGroupCount(c:GetControler(),LOCATION_ONFIELD,0,nil)==1
			and Duel.GetFieldGroupCount(c:GetControler(),0,LOCATION_ONFIELD,nil)>1
	end
...
if not c72589042 then
	...
	function c72589042.condition(e,tp,eg,ep,ev,re,r,rp)
		return Duel.GetAttacker():IsControler(1-tp) and Duel.GetMatchingGroupCount(nil,tp,LOCATION_ONFIELD+LOCATION_HAND,0,e:GetHandler())==1
	end
...
if not c42338879 then
	...
	function c42338879.spcon(e,c)
		...
		return (Duel.GetFieldGroupCount(tp,LOCATION_MZONE,LOCATION_MZONE)==0
			or (Duel.GetFieldGroupCount(tp,LOCATION_ONFIELD,0)==1 and Duel.GetFieldGroupCount(tp,0,LOCATION_MZONE)>0))
...
if not c38007744 then
	...
	function c38007744.spcon(e,c)
		if c==nil then return true end
		return Duel.GetFieldGroupCount(c:GetControler(),LOCATION_ONFIELD,0)==1
			and Duel.GetFieldGroupCount(c:GetControler(),0,LOCATION_ONFIELD)>1
...
if not c4647954 then
	...
	function c4647954.spcon(e,c)
		...
		return Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0 and
			Duel.GetFieldGroupCount(c:GetControler(),LOCATION_ONFIELD,0)==1
	end
...
if not c58655504 then
	...
	function c58655504.condition(e,tp,eg,ep,ev,re,r,rp)
		return eg:IsExists(Card.IsSummonPlayer,1,nil,1-tp) and (Duel.GetFieldGroupCount(tp,LOCATION_ONFIELD,0)==1)
	end
...
if not c2295831 then
	...
	function c2295831.regcon(e,tp,eg,ep,ev,re,r,rp)
		local c=e:GetHandler()
		return Duel.GetFieldGroupCount(tp,LOCATION_ONFIELD,0)==1 and Duel.GetFieldGroupCount(tp,0,LOCATION_MZONE)>=3
	...
	function c74414885.spop(e,tp,eg,ep,ev,re,r,rp)
		...
		if #g>0 then
			for tc in aux.Next(g) do
				...
				--Lizard check
				local e6=aux.createContinuousLizardCheck(c,LOCATION_MZONE,c74414885.lizfilter)
...
if not c69838761 then
	...
	function c69838761.ntcon(e,c,minc)
		if c==nil then return true end
		return minc==0 and c:IsLevelAbove(5) and Duel.GetMatchingGroupCount(Card.IsFaceup,e:GetHandlerPlayer(),LOCATION_ONFIELD,0,1,nil)==1
	end
...
if not c14532163 then
	...
	function c14532163.descon(e,tp,eg,ep,ev,re,r,rp)
		return Duel.GetMatchingGroupCount(Card.IsFaceup,tp,LOCATION_ONFIELD,0,1,nil)==1
	end
...
if not c32588805 then
	...
	function c32588805.setcon(e,tp,eg,ep,ev,re,r,rp)
		return e:GetHandler():IsSummonType(SUMMON_TYPE_TRIBUTE)
			and Duel.GetMatchingGroupCount(Card.IsSpellTrap,tp,LOCATION_ONFIELD,0,1,nil)==1
	end
...
if not c7304544 then
	...
	function c7304544.condition(e,tp,eg,ep,ev,re,r,rp)
		return tp==Duel.GetTurnPlayer() and Duel.GetMatchingGroupCount(c7304544.filter,tp,LOCATION_ONFIELD,0,1,nil)==1
...
if not c12538374 then
	...
	c12538374.listed_names={12538374}
	...
	function c12538374.condition(e,tp,eg,ep,ev,re,r,rp)
		return tp==Duel.GetTurnPlayer() and Duel.GetMatchingGroupCount(c12538374.filter,tp,LOCATION_ONFIELD,0,1,nil)==1
	end
	...
	function c12538374.operation(e,tp,eg,ep,ev,re,r,rp)
		if e:GetHandler():IsRelateToEffect(e) and Duel.GetMatchingGroupCount(c12538374.filter,tp,LOCATION_ONFIELD,0,1,nil)==1 then
			Duel.SpecialSummon(e:GetHandler(),0,tp,tp,false,false,POS_FACEUP)
		end
	end
end
if not c5592689 then
	...
	function c5592689.sscon(e,tp,eg,ep,ev,re,r,rp)
		return tp==Duel.GetTurnPlayer() and Duel.GetMatchingGroupCount(c5592689.filter,tp,LOCATION_ONFIELD,0,1,nil)==1
	end
	...
	function c5592689.ssop(e,tp,eg,ep,ev,re,r,rp)
		if e:GetHandler():IsRelateToEffect(e) and Duel.GetMatchingGroupCount(c5592689.filter,tp,LOCATION_ONFIELD,0,1,nil)==1 then
...
if not c14883228 then
	...
	function c14883228.handcon(e)
		local tp=e:GetHandlerPlayer()
		return Duel.GetMatchingGroupCount(Card.IsSpellTrap,tp,LOCATION_ONFIELD,0,1,nil)==1
...
if not c82697249 then
	...
	function c82697249.imcon(e)
		return Duel.GetFieldGroupCount(e:GetHandlerPlayer(),LOCATION_ONFIELD,0)==2
	end
...
if not c33611061 then
	...
	function c33611061.condition(e,tp,eg,ep,ev,re,r,rp)
		return Duel.GetMatchingGroupCount(nil,tp,LOCATION_ONFIELD,0,1,e:GetHandler())==1
	end
